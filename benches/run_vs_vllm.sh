#!/usr/bin/env bash
# Pie vs vLLM Benchmark Runner
#
# Usage:
#   ./benches/run_vs_vllm.sh                          # Full suite
#   ./benches/run_vs_vllm.sh --tiers 1a,1b            # Tier 1 only
#   ./benches/run_vs_vllm.sh --pie-only               # Skip vLLM
#   ./benches/run_vs_vllm.sh --vllm-only              # Skip Pie
#
# Prerequisites:
...
echo "=== Pie vs vLLM Benchmark Suite ==="
echo ""
...
python3 "$SCRIPT_DIR/bench_vs_vllm.py" \
    --output-json "$RESULTS_DIR/vs_vllm.json" \
    "$@"

echo ""
echo "Results saved to: $RESULTS_DIR/vs_vllm.json"
