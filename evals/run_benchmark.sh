...
# Init config if it doesn't exist (downloads Python runtime)
if [[ ! -f "$HOME/.pie/config.toml" ]]; then
    uv run pie config init
fi

# Parse device list for config update
# "cuda:0,cuda:1" → --device "cuda:0,cuda:1" (pie config update handles the split)
uv run pie config update \
    --hf-repo "$MODEL" \
    --device "$DEVICE" \
    --disable-auth \
    --port "$PIE_PORT" 2>&1 | tail -5

log "Starting Pie server..."
uv run pie serve --no-auth --port "$PIE_PORT" &
PIE_PID=$!
